...
\noindent A state of the art skin detection method has been recently developed by \cite{brancati:17}. Here, we review the method and extend it adding more rules to enforce the constraints and seeking for a better accuracy in terms of false positive rate without hurting the performance of the original method. We also show an implementation of a grid search to figure out the best combination of parameters to compute the trapezoids coordinates.
