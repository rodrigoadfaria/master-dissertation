...
\noindent ou ainda, efetuando uma mudança de coordenadas subtraindo-se as cores primárias R, G e B da cor branca $W = (1, 1, 1)$ \citep{gonzalez:02}:
\begin{equation}
  \begin{bmatrix}
    C \\ M \\ Y
  \end{bmatrix} = 
...
\begin{exmp}
Exemplo de conjunto \emph{fuzzy} sobre a variável de base altura. Aqui, tem-se a representação tradicional com conjuntos clássicos, onde a transição é abrupta, e a representação com conjuntos \emph{fuzzy}.
\begin{figure}[h]
    \centering
    ...
    \end{minipage}
    \caption[Representação da variável altura em conjuntos clássicos e \emph{fuzzy}]{Representação da variável altura em conjuntos clássicos e fuzzy. À esquerda tem-se a representação de conjuntos clássicos; à direita a representação com conjuntos fuzzy. Fonte: proposto pelo autor.}
    \label{fig:crisp_fuzzy_set}
\end{figure}
\end{exmp}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.8\textwidth]{fuzzy_definitions}
...
Seja dois conjuntos \emph{fuzzy} $A$ e $B$, definidos sobre um mesmo universo de discurso $U$. Utilizando as funções $max$ e $min$, tem-se as seguintes definições \citep{klir:95}:
...
Além desses operadores, existem outros que podem ser utilizados para efetuar as operações lógicas. Um deles é a norma triangular, também denominada \emph{t-norma}. Outro é a conorma triangular, usualmente denominada por \emph{t-conorma} ou \emph{s-norma}. Os operadores utilizados para a intersecção de conjuntos \emph{fuzzy} pertencem à classe \emph{t-norma}. Os operadores utilizados para a união de conjuntos \emph{fuzzy} pertencem à classe \emph{t-conorma} ou \emph{s-norma} \citep{zimmermann:01}.

Ambas \emph{t-norma} e \emph{s-norma} são funções binárias do tipo $[0, 1] \times [0, 1] \rightarrow [0, 1]$, que devem satisfazer algumas propriedades, tais como, monotonicidade, comutatividade, associatividade e condições de contorno \footnote{As propriedades não serão expressas neste trabalho, embora sejam amplamente conhecidas, exceto pelas condições de contorno. Mais detalhes podem ser vistos em \citet{zimmermann:01}.}. Os operadores $max$ e $min$ satisfazem tais propriedades e, por essa razão, podem ser utilizados nas operações lógicas \emph{fuzzy} \citep{zimmermann:01}.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.6\textwidth]{funcao_triangular}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.6\textwidth]{funcao_trapezoidal}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.6\textwidth]{funcao_gaussiana}
